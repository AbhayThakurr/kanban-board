...
import CardStatus from "../Card/CardStatus.jsx";
...
const Status = () => {
  ...
  const count = useCallback(() => {
    const groupedTickets = tick.reduce(
      (acc, ticket) => {
    ...
    const { todopre, donepre, cancelledpre, backlogpre, inprogresspre } =
      groupedTickets;

    const sortedGroups =
      order === "Title"
        ? {
            todopre: [...todopre].sort((a, b) =>
              a.title.localeCompare(b.title)
            ),
            inprogresspre: [...inprogresspre].sort((a, b) =>
              a.title.localeCompare(b.title)
            ),
            backlogpre: [...backlogpre].sort((a, b) =>
              a.title.localeCompare(b.title)
            ),
            donepre: [...donepre].sort((a, b) =>
              a.title.localeCompare(b.title)
            ),
            cancelledpre: [...cancelledpre].sort((a, b) =>
              a.title.localeCompare(b.title)
            ),
          }
        : {
            todopre: [...todopre].sort((a, b) => b.priority - a.priority),
            inprogresspre: [...inprogresspre].sort(
              (a, b) => b.priority - a.priority
            ),
            backlogpre: [...backlogpre].sort((a, b) => b.priority - a.priority),
            donepre: [...donepre].sort((a, b) => b.priority - a.priority),
            cancelledpre: [...cancelledpre].sort(
              (a, b) => b.priority - a.priority
            ),
          };
  ...
  const renderCards = (tickets, status) => {
    return tickets.map((ticket) => {
      ...
      return (
        <CardStatus key={ticket.id} ticket={ticket} available={available} />
      );
    });
  };
  ...
  const renderBoard = (title, image, tickets, status) => (
    <div className="Board">
      ...
      <div className="Cards">{renderCards(tickets, status)}</div>
    </div>
  );
